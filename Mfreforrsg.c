...
void writedata(float **output,char *filename,int xn,int tn)
{
	FILE *fp;
...
/*  
  ...
  float **vxdataabo,**vzdataabo,**pdatabel;
  vxdataabo=sf_floatalloc2(tn,xn-2*(pml+N));
  vzdataabo=sf_floatalloc2(tn,xn-2*(pml+N));
  pdatabel=sf_floatalloc2(tn,xn-2*(pml+N));
  zero_float_2d(vxdataabo,tn,xn-2*(pml+N));
  zero_float_2d(vzdataabo,tn,xn-2*(pml+N));
  zero_float_2d(pdatabel,tn,xn-2*(pml+N));
  float **txzdataabo,**txzdatabel,**txzdatabel2;
  txzdataabo=sf_floatalloc2(tn,xn-2*(pml+N));
  txzdatabel=sf_floatalloc2(tn,xn-2*(pml+N));
  txzdatabel2=sf_floatalloc2(tn,xn-2*(pml+N));
  zero_float_2d(txzdataabo,tn,xn-2*(pml+N));
  zero_float_2d(txzdatabel,tn,xn-2*(pml+N));
  zero_float_2d(txzdatabel2,tn,xn-2*(pml+N));
  float **vxdataabo2,**pdatabel2,**vzdataabo2;
  vxdataabo2=sf_floatalloc2(tn,xn-2*(pml+N));
  pdatabel2=sf_floatalloc2(tn,xn-2*(pml+N));
  vzdataabo2=sf_floatalloc2(tn,xn-2*(pml+N));
  zero_float_2d(vxdataabo2,tn,xn-2*(pml+N));
  zero_float_2d(vzdataabo2,tn,xn-2*(pml+N));
  zero_float_2d(pdatabel2,tn,xn-2*(pml+N));
  float **txxdatabel,**txxdatabel2,**tzzdatabel,**tzzdatabel2;
  txxdatabel=sf_floatalloc2(tn,xn-2*(pml+N));
  txxdatabel2=sf_floatalloc2(tn,xn-2*(pml+N));
  tzzdatabel=sf_floatalloc2(tn,xn-2*(pml+N));
  tzzdatabel2=sf_floatalloc2(tn,xn-2*(pml+N));
  zero_float_2d(txxdatabel,tn,xn-2*(pml+N));
  zero_float_2d(tzzdatabel,tn,xn-2*(pml+N));
  zero_float_2d(txxdatabel2,tn,xn-2*(pml+N));
  zero_float_2d(tzzdatabel2,tn,xn-2*(pml+N));
*/  
  ...
  //for(int shot=0;shot<shotnum;shot++) //multi-shot begin
  {
     ...
     for(t=0;t<tn;t++)
     {
	   ...
	   if(t%3==0){
		   if((fp=fopen("vzsnapshot.dat","ab+"))!=NULL){
			   for(int i=pml+N;i<xn-pml-N;i++)
			       for(int j=5;j<zn-pml-N;j++)
				       fwrite(&vz[i][j],sizeof(float),1,fp);
		       fclose(fp);
	       }
	   }
       ...
       { 
         ...
         pdata[i][t] -=(tzz[i+pml+N][re_dep-1]+tzz[i+pml+N+1][re_dep-1]+txx[i+pml+N][re_dep-1]+txx[i+pml+N+1][re_dep-1])/4;
/*
		  //temporary---remember to delete
		 vxdataabo[i][t]=vx[i+pml+N][re_dep-1];
		 vzdataabo[i][t]=vz[i+pml+N][re_dep-1];
         pdatabel[i][t] -=(tzz[i+pml+N][re_dep]+tzz[i+pml+N+1][re_dep]+txx[i+pml+N][re_dep]+txx[i+pml+N+1][re_dep])/4;
		 txzdataabo[i][t]=txz[i+pml+N][re_dep-1];
		 txzdatabel[i][t]=txz[i+pml+N][re_dep];
		 txzdatabel2[i][t]=txz[i+pml+N][re_dep+1];
		 vxdataabo2[i][t]=vx[i+pml+N][re_dep-2];
		 vzdataabo2[i][t]=vz[i+pml+N][re_dep-2];
         pdatabel2[i][t] -=(tzz[i+pml+N][re_dep+1]+tzz[i+pml+N+1][re_dep+1]+txx[i+pml+N][re_dep+1]+txx[i+pml+N+1][re_dep+1])/4;
		 txxdatabel[i][t]=(txx[i+pml+N][re_dep]+txx[i+pml+N+1][re_dep])/2;	
		 txxdatabel2[i][t]=(txx[i+pml+N][re_dep+1]+txx[i+pml+N+1][re_dep+1])/2;	
		 tzzdatabel[i][t]=(tzz[i+pml+N][re_dep]+tzz[i+pml+N+1][re_dep])/2;	
		 tzzdatabel2[i][t]=(tzz[i+pml+N][re_dep+1]+tzz[i+pml+N+1][re_dep+1])/2;	
*/
       }
...
/*
	 sprintf(filename,"vxdata.txt");	 writedata(vxdata,filename,xn-2*(pml+N),tn);
	 sprintf(filename,"vzdata.txt");	 writedata(vzdata,filename,xn-2*(pml+N),tn);
	 sprintf(filename,"pdata.txt");		 writedata(pdata,filename,xn-2*(pml+N),tn);
	 sprintf(filename,"vxdataabo.txt");  writedata(vxdataabo,filename,xn-2*(pml+N),tn);
	 sprintf(filename,"vzdataabo.txt");  writedata(vzdataabo,filename,xn-2*(pml+N),tn);
	 sprintf(filename,"pdatabel.txt");   writedata(pdatabel,filename,xn-2*(pml+N),tn);
	 sprintf(filename,"txzdataabo.txt"); writedata(txzdataabo,filename,xn-2*(pml+N),tn);
	 sprintf(filename,"txzdatabel.txt"); writedata(txzdatabel,filename,xn-2*(pml+N),tn);
	 sprintf(filename,"txzdatabel2.txt");writedata(txzdatabel2,filename,xn-2*(pml+N),tn);
	 sprintf(filename,"vxdataabo2.txt"); writedata(vxdataabo2,filename,xn-2*(pml+N),tn);
	 sprintf(filename,"vzdataabo2.txt"); writedata(vzdataabo2,filename,xn-2*(pml+N),tn);
	 sprintf(filename,"pdatabel2.txt");  writedata(pdatabel2,filename,xn-2*(pml+N),tn);
	 sprintf(filename,"txxdatabel.txt"); writedata(txxdatabel,filename,xn-2*(pml+N),tn);
	 sprintf(filename,"tzzdatabel.txt"); writedata(tzzdatabel,filename,xn-2*(pml+N),tn);
	 sprintf(filename,"txxdatabel2.txt"); writedata(txxdatabel2,filename,xn-2*(pml+N),tn);
	 sprintf(filename,"tzzdatabel2.txt"); writedata(tzzdatabel2,filename,xn-2*(pml+N),tn);
*/
...
/*
  //temporary---remember to delete
  free_float_2d(txxdatabel); free_float_2d(txxdatabel2);
  free_float_2d(tzzdatabel); free_float_2d(tzzdatabel2);
  free_float_2d(vxdataabo); free_float_2d(vzdataabo); free_float_2d(pdatabel);
  free_float_2d(txzdataabo);free_float_2d(txzdatabel);free_float_2d(txzdatabel2);
  free_float_2d(vxdataabo2);free_float_2d(vzdataabo2);free_float_2d(pdatabel2);
*/
