...
//	}
	...
	free_float_2d(vzphase);
	free_float_2d(phphase);
}
void  amplitude_cpz(sf_complex **vzdatafk,sf_complex **phdatafk,float *pha_cpz,float *amp_cpz)
{
	utils_print_title("calculate the amplitude spectrum");

	float **vzphase,**phphase,**vzampli,**phampli;
	vzphase=sf_floatalloc2(nt,nx);
	phphase=sf_floatalloc2(nt,nx);
	vzampli=sf_floatalloc2(nt,nx);
	phampli=sf_floatalloc2(nt,nx);
	zero_float_2d(vzphase,nt,nx);
	zero_float_2d(phphase,nt,nx);
	zero_float_2d(vzampli,nt,nx);
	zero_float_2d(phampli,nt,nx);
	for(int i=0;i<nx;i++)
		for(int j=0;j<nt;j++){
			vzphase[i][j]=atan(cimag(vzdatafk[i][j])/creal(vzdatafk[i][j]));
			phphase[i][j]=atan(cimag(phdatafk[i][j])/creal(phdatafk[i][j]));
			vzampli[i][j]=sqrt(powf(cimag(vzdatafk[i][j]),2.0)+powf(creal(vzdatafk[i][j]),2.0));
			phampli[i][j]=sqrt(powf(cimag(phdatafk[i][j]),2.0)+powf(creal(phdatafk[i][j]),2.0));
		}
	...
	for(int j=nt/2;j<nt;j++)
		for(int i=0;i<nx;i++){
			...
			if(tmp1>0) qp=sqrt(tmp1); else qp=.0;
			if(absolutevalue(pp)<0.55){
				tmp2=powf((-qp*phampli[i][j]*cos(phphase[i][j])+dens*amp_cpz[j]*vzampli[i][j]*cos(pha_cpz[j]+vzphase[i][j])),2.0);
				tmp3=powf((-qp*phampli[i][j]*sin(phphase[i][j])+dens*amp_cpz[j]*vzampli[i][j]*sin(pha_cpz[j]+vzphase[i][j])),2.0);
				tmp4=powf((qp*phampli[i][j]*cos(phphase[i][j])+dens*amp_cpz[j]*vzampli[i][j]*cos(pha_cpz[j]+vzphase[i][j])),2.0);
				tmp5=powf((qp*phampli[i][j]*sin(phphase[i][j])+dens*amp_cpz[j]*vzampli[i][j]*sin(pha_cpz[j]+vzphase[i][j])),2.0);
				Epz_amplitude+=powf(sqrtf(tmp2+tmp3)+sqrtf(tmp4+tmp5),2.0);
			}
		}
	...
	while(Epz_amplitude>ee&&index<numiteration){
		...
		for(int j=nt/2;j<nt;j++)
			for(int i=0;i<nx;i++){
				...
				if(tmp1>0) qp=sqrt(tmp1); else qp=.0;
				if(absolutevalue(pp)<0.55){
					tmp2=(-qp*phampli[i][j]*cos(phphase[i][j])+dens*amp_cpz[j]*vzampli[i][j]*cos(pha_cpz[j]+vzphase[i][j]));
					tmp3=(-qp*phampli[i][j]*sin(phphase[i][j])+dens*amp_cpz[j]*vzampli[i][j]*sin(pha_cpz[j]+vzphase[i][j]));
					tmp4=(qp*phampli[i][j]*cos(phphase[i][j])+dens*amp_cpz[j]*vzampli[i][j]*cos(pha_cpz[j]+vzphase[i][j]));
					tmp5=(qp*phampli[i][j]*sin(phphase[i][j])+dens*amp_cpz[j]*vzampli[i][j]*sin(pha_cpz[j]+vzphase[i][j]));
					if(sqrtf(tmp2*tmp2+tmp3*tmp3)!=0.0)
						gradient[j]+=2.0*(sqrtf(tmp2*tmp2+tmp3*tmp3)+sqrtf(tmp4*tmp4+tmp5*tmp5))*
									(0.5/sqrtf(tmp2*tmp2+tmp3*tmp3)*(2.0*tmp2*dens*vzampli[i][j]*cos(pha_cpz[j]+vzphase[i][j])
										                             +2.0*tmp3*dens*vzampli[i][j]*sin(pha_cpz[j]+vzphase[i][j]))+
									 0.5/sqrtf(tmp4*tmp4+tmp5*tmp5)*(2.0*tmp4*dens*vzampli[i][j]*cos(pha_cpz[j]+vzphase[i][j])
										                             +2.0*tmp5*dens*vzampli[i][j]*sin(pha_cpz[j]+vzphase[i][j])));
				}
...
//		sf_warning("amp_cpz=%f",amp_cpz[i]);
	...
	free_float_2d(vzphase); free_float_2d(vzampli);
	free_float_2d(phphase); free_float_2d(phampli);
}
