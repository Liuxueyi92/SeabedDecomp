...
/*
	for(int i=0;i<nx;i++)
		for(int j=absolutevalue(i-390)*5.6410+800;j<nt;j++){
			vxdata[i][j]=0.0;
			vzdata[i][j]=0.0;
			phdata[i][j]=0.0;
		}
*/
	...
	//for testing ====== Warning:remember delete it====//
	for(int i=0;i<nx;i++)
		for(int j=0;j<nt;j++){
			uppts[i][j]=vxdata[i][j];
			dopts[i][j]=vzdata[i][j];
			upsts[i][j]=phdata[i][j];
			dosts[i][j]=0.0;
		}//=====:remember delete it====//
...
/*	for(int j=nt/2;j<nt;j++){
		for(int i=0;i<nx/2;i++){
					uptzzfk[i][j]=phdatafk[i][j];
					dotzzfk[i][j]=vzdatafk[i][j];
		}
	}
*/	
}
...
void  phase_cpz(sf_complex **vzdatafk,sf_complex **phdatafk,float *pha_cpz)
{
	...
	float step=0.0001; //step of one iteration (units is degree)
...
void  amplitude_cpz(sf_complex **vzdatafk,sf_complex **phdatafk,float *pha_cpz,float *amp_cpz)
{
	...
	float ee=0.0000005;
	int index=0,numiteration=10000;
	float *gradient=sf_floatalloc(nt);
	float step=0.00000000000000002;
